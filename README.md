...
- **Limit Computation** — left-hand, right-hand, and two-sided limits via Richardson extrapolation
 ...
 - **Arc Length** — approximate the arc length of a curve over any interval using adaptive chord summation
  ...
  - **Taylor Series Expansion** — expand any function into a Taylor polynomial at an arbitrary point, with configurable order, coefficient display, and comparison plot of Taylor vs. original
   ...
   - **ODE Method Comparison** — compare different numerical methods (Euler, Improved Euler, Midpoint, RK4, RKF45) side-by-side for the same ODE problem, with overlaid solution plots to visualize accuracy and convergence differences. Available on desktop (Python), Android (Java), and web landing page interactive demo.
   ...
   - **Vector Field Plotter** — visualize 2D autonomous systems dx/dt=P(x,y), dy/dt=Q(x,y) as a vector field with colored arrows, overlay solution curves from initial conditions using RK4. Available on desktop (Python), Android (Java), and web landing page.
   - **Custom Function Definition** — define user-named functions like `f(x) = x^2 + 1` and reference them in any expression as `f(expr)`. Supports arbitrary nesting and composition. Available on all platforms (C core, Android, Python, Web).
   - **Calculation History** — automatically record the most recent 10 calculations (expressions and results), persist via SharedPreferences on Android. Supports clearing history, retrieving the last expression, and exporting history as CSV. Available on all platforms (C core, JNI, Java, Python).
 ...
 - **Interactive Plot** — Matplotlib toolbar for zoom, pan, and export
  ...
  - **Unit Converter** — convert between different units of measurement including Length, Weight, Temperature, Area, Volume, Time, Data Storage, Speed, and Angle. Supports 9 unit categories with comprehensive conversion factors.
   ...
   - **CSV Data Import & Scatter Plot** — import CSV/TSV data files with configurable delimiters (comma, tab, semicolon, space) and column selection. Plot data as scatter, line, or bar charts. Fit trendlines (Linear, Quadratic, Exponential) with R² goodness-of-fit display. Export data points and trendline predictions as CSV. Export plots as PNG. Available on desktop (Python) and Android (Java). Interactive demo on the project landing page.
   ...
   - **Base Number Converter** — convert numbers between binary (2), octal (8), decimal (10), hexadecimal (16), and any base from 2 to 36. Supports both single-base conversion and simultaneous display of all common bases. Interactive demo available on the project landing page. Available on both desktop (Python) and Android (Java).
    ...
    - **Convolution Calculator** — compute the discrete 1D convolution of two numeric sequences. Enter two comma-separated sequences and get the full convolution result c[n] = Σ a[k]·b[n-k]. Useful for signal processing, polynomial multiplication, and filter design. Available on desktop (Python), Android (Java), and web landing page interactive demo.
   ...
   - **Data Interpolation Calculator** — fit data points using six interpolation methods: Linear, Polynomial (Lagrange), Polynomial (Newton), Cubic Spline, Natural Spline (S''(x₀)=S''(xₙ)=0), and Akima Spline. Evaluate the interpolated function at any x, visualize the interpolation curve with data points. Export interpolated curve data to CSV. Available on desktop (Python), Android (Java), and web landing page interactive demo.
...
- **Convolution Calculator** — compute the discrete 1D convolution of two numeric sequences (comma-separated). The convolution c[n] = Σ a[k]·b[n-k] is a fundamental operation in signal processing, polynomial multiplication, and filter design. Available on desktop (Python/C), Android (Java/JNI), and web landing page interactive demo.
...
- **Data Interpolation Calculator** — fit data points using six interpolation methods: Linear, Polynomial (Lagrange), Polynomial (Newton), Cubic Spline, Natural Spline (S''(x₀)=S''(xₙ)=0), and Akima Spline. Evaluate the interpolated function at any x, visualize the interpolation curve with data points annotated. Export interpolated curve data to CSV. Interactive demo on the project landing page. Available on desktop (Python), Android (Java), and web landing page interactive demo.
...
- **CSV Data Import & Scatter Plot** — import CSV/TSV data files with configurable delimiters (comma, tab, semicolon, space) and column selection. Plot data as scatter, line, or bar charts. Fit trendlines (Linear, Quadratic, Exponential) with R² goodness-of-fit display. Export data points and trendline predictions as CSV. Export plots as PNG. Available on both desktop (Python) and Android (Java). Interactive demo on the project landing page.
...
- **Vector Field Plotter** — visualize 2D autonomous systems dx/dt=P(x,y), dy/dt=Q(x,y) as a vector field with magnitude-colored arrows, overlay solution curves from initial conditions using RK4 solver. Supports customizable grid size, range, and initial conditions. Available on desktop (Python), Android (Java), and web landing page interactive demo.
